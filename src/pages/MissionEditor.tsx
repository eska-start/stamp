    ...
    <div style={{ minHeight: '100vh', paddingBottom: 32, background: C.bg }}>
      ...
      <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ background: C.card, borderRadius: 20, padding: 16, boxShadow: SH.card }}>
          ...
          {templates.length === 0 && (
            <div style={{ textAlign: 'center', color: C.t3, padding: '16px 0', fontSize: 14 }}>미션이 없습니다. 아래에서 추가해주세요!</div>
          )}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {templates.map(t => (
              <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 14, background: C.soft }}>
                <span style={{ fontSize: 22 }}>{t.icon}</span>
                <span style={{ flex: 1, fontWeight: 700, color: C.t1, fontSize: 14 }}>{t.title}</span>
                <button
                  onClick={() => handleRemove(t.id)}
                  style={{ width: 28, height: 28, borderRadius: 8, background: '#FFE5E5', color: '#E53535', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700 }}>
                  ✕
                </button>
              </div>
            ))}
        ...
        <div style={{ background: C.card, borderRadius: 20, padding: 16, boxShadow: SH.card }}>
              ...
              {ICONS.map(icon => (
                <button
                  ...
                  style={{
                    ...
                    background: newIcon === icon ? C.accentSoft : C.soft,
                    border: newIcon === icon ? `2px solid ${C.accent}` : '2px solid transparent',
                    cursor: 'pointer',
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              onClick={handleAdd}
              style={{ padding: '10px 18px', borderRadius: 12, background: C.accent, color: '#fff', fontWeight: 900, fontSize: 14, border: 'none', cursor: 'pointer', boxShadow: SH.btn }}>
              추가
            </button>
          </div>
        </div>
